##############################################################################
# Enoch Blog WordPress Infrastructure - Main Configuration
##############################################################################
...
##############################################################################
# IAM Roles - Management Account SSM + Dev Account EC2
##############################################################################

# SSM Access Role (Management Account)
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:ssm:${var.environment_config.aws_region}:${var.environment_config.management_account_id}:parameter/*"
      },
      {
        Effect = "Allow"
        Action = [
...
# EC2 Policy - Assume SSM Role
resource "aws_iam_role_policy" "assume_ssm_role" {
  name = "${var.environment_config.project_name}-assume-ssm-policy"
...
resource "aws_ssm_parameter" "efs_id" {
  ...
  tags = {
    ...
    Application = "EnochBlog"
  }
}

# RDS Endpoint - Not needed, passed via templatefile

# Database Name
resource "aws_ssm_parameter" "db_name" {
  provider = aws.management

  name        = "${var.iam_config.ssm_parameter_path_prefix}/db-name"
...
resource "aws_ssm_parameter" "db_username" {
  ...
  name        = "${var.iam_config.ssm_parameter_path_prefix}/db-username"
...
resource "aws_ssm_parameter" "db_password" {
  ...
  name        = "${var.iam_config.ssm_parameter_path_prefix}/db-password"
  description = "${var.environment_config.project_name} Database Password"
  type        = "SecureString"
  value       = var.database_config.db_password
  overwrite   = true
...
resource "aws_ssm_parameter" "alb_dns" {
  ...
  name        = "${var.iam_config.ssm_parameter_path_prefix}/alb-dns"
  description = "${var.environment_config.project_name} ALB DNS Name"
  type        = "String"
  value       = aws_lb.main.dns_name
  overwrite   = true

  tags = {
    Name        = "${var.environment_config.project_name}-alb-dns"
    Application = "EnochBlog"
  }
}
